<div>
            ...
            <div wire:loading.class='opacity-50' @class([
                ...
                @forelse ($categories as $key => $category)
                    <div wire:key='wkw-{{ $key }}'>
                        ...
                        <x-cards.category id="{{ $category->id }}" name="{{ $category->name }}"
                            url="{{$category->getPhotoOrDefaultUrl()}}"
                ...
                <div
                    ...
                    <div class="flex flex-col gap-4">
                        ...
                        <div class="flex gap-6 2xl:gap-6 lg:gap-4">
                            ...
                            <!-- File Upload -->
                            <x-file-upload id="category.image" wire:model='image' :error="$errors->first('image')">

                                <span wire:loading.class='hidden' wire:target='image'>
                                    <img class="w-24 h-24 border rounded-full shadow-sm border-m-orange shadow-m-orange-l lg:w-26 lg:h-26 2xl:h-32 2xl:w-32"
                                        src="
                                        {{ isset($image) ? $image->temporaryUrl() : (isset($imageUrl) ? $imageUrl : asset('img/avatars/category.png')) }}">
                                </span>
                            ...
                            <div class="flex flex-col flex-grow gap-4 2xl:gap-4 lg:gap-2">
                                ...
                                <x-input.group inline label="{{ __('fields.description') }}" for='supplier.description'
                                    :error="$errors->first('editing.description')">

                                    <x-jet-input wire:model.defer='editing.description' id="supplier.description"
                                        class="block w-full" type="text" name="editing.description"
                                        placeholder="{{ __('placeholders.enter-description') }}" :value="old('editing.description')" />
                        ...
                        <div class="flex justify-end gap-4 pt-4">
                            ...
                            <x-jet-button wire:click='saveCategory' class="p-2 w-28">
                                <span class="mx-auto">
                                    {{ __('fields.save') }}
                                </span>
                            </x-jet-button>

                            <x-jet-secondary-button wire:click='resetCategory' class="p-2 w-28">
                                <span class="mx-auto">
                                    {{ __('fields.cancel') }}
                                </span>
    ...
    <!-- Modals -->
    <x-modals.confirmation action='deleteCategory' model='showDelete' title="{{ __('fields.delete-category') }}" />

    <x-modals.content action='saveCategory' model='showEditModal' title="{{ __('fields.edit-category') }}">
        ...
        <x-input.group inline label="{{ __('fields.name') }}" for='category.name' :error="$errors->first('editing.name')">

            <x-jet-input wire:model.defer='editing.name' id="category.name" class="block w-full" type="text"
