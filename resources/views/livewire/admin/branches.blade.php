<div>
            ...
            <div wire:loading.class='opacity-50' @class([
                ...
                @forelse ($branches as $key => $branch)
                    <div wire:key='wkw-{{ $key }}'>
                        <x-cards.branch id="{{ $branch->id }}" name="{{ $branch->name }}" editAction='showEdit'
            ...
            <div class="flex-col items-center flex-grow hidden w-1/2 gap-4 overflow-hidden lg:gap-2 2xl:gap-4 lg:flex">
                ...
                <div
                    ...
                    <div class="flex flex-col gap-4">
                        ...
                        <x-input.group inline label="{{ __('fields.name') }}" for='branch.name' :error="$errors->first('editing.name')">

                            <x-jet-input wire:model.defer='editing.name' id="branch.name" class="block w-full"
                        ...
                        <div class="flex justify-center gap-4 pt-4">
                            ...
                            <x-jet-button wire:click='saveBranch' class="p-2 w-28">
                                <span class="mx-auto">
                                    {{ __('fields.save') }}
                                </span>
                            </x-jet-button>

                            <x-jet-secondary-button wire:click='resetBranch' class="p-2 w-28">
                                <span class="mx-auto">
                                    {{ __('fields.cancel') }}
                                </span>
    ...
    <!-- Modals -->
    <x-modals.confirmation action='deleteBranch' model='showDelete' title="{{ __('fields.delete-branch') }}" />

    <x-modals.content action='saveBranch' model='showEditModal' title="{{ __('fields.edit-branch') }}">

        <x-input.group inline label="{{ __('fields.name') }}" for='branch.name' :error="$errors->first('editing.name')">

            <x-jet-input wire:model.defer='editing.name' id="branch.name" class="block w-full" type="text"
