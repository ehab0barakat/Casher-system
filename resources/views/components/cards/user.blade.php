@props(['id', 'name', 'branch', 'username', 'permissions', 'editAction', 'deleteAction'])
<div
    ...
    <div class="flex items-center gap-4">
        ...
        <!-- Avatar -->
        <img class="h-16 border rounded-full border-m-blue/30 lg:h-20 2xl:h-24 lg:w-20 2xl:w-24" src="{{ asset('img/avatars/person.png') }}"
            alt="avatar-man">

        <!-- Title / Subtitle / Sub-subtitle-->
        <div class="flex flex-col items-start justify-center gap-2">

            <span class="lg:text-lg 2xl:text-xl font-roboto-m">{{ $name . ' (' . $branch . ')' }}</span>

            <span class="text-sm lg:text-base 2xl:text-lg font-roboto">{{ $username }}</span>

            <span class="text-sm 2xl:text-base font-roboto-l"><span class="text-m-orange font-roboto-b">[</span>
                {{ $permissions }}
                <span class="text-m-orange font-roboto-b"> ]</span></span>
